...
control "SV-218033" do
  ...
  desc "default", "The hash on important files like audit system executables should match
...
hashes could be a sign of nefarious activity on the system."
  desc "check", "The following command will list which audit files on the system have file hashes different from what is expected by the RPM database. 

# rpm -V audit | awk '$1 ~ /..5/ && $2 != \"c\"'


If there is output, this is a finding."
  desc "fix", "The RPM package management system can check the hashes of audit system package files. Run the following command to list which audit files on the system have hashes that differ from what is expected by the RPM database: 
